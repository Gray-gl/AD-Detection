    ...
    for sent_text in subject_text:
        ...
        if len(current_window) + len(sent_tokenized) <= max_token_len:
            ...
            tokenized_windows.append(
                tokenizer.encode(current_window, add_special_tokens=True, max_length=max_len, truncation=True))
            current_window = []
...
def read_input_text_len_control(df, sample_size=-1, max_len=128, token_cut=True):
    if token_cut:
        ...
        tokenizer, _ = load_transformer_model_tokenizer(ppb.BertModel, ppb.BertTokenizer,
                                                        '/project_bdda5/bdda/ywang/class_ncd/new_models/bert-base-uncased')
    else:
    ...
    if sample_size == -1:
        ...
        indexes_list = [[i * sample_size, (i + 1) * sample_size] for i in
                        range(all_number // sample_size)]  # + [[all_number-all_number%sample_size, all_number]]
        list_output_dfs = [texts_df[["id", "joined_all_par_trans", "ad"]].iloc[i1:i2, :].reset_index(drop=True) for
                           [i1, i2] in indexes_list]
...
def read_input_no_len_control(df, model, sample_size=-1, max_len=512, save_trans=None):
    if save_trans is not None:
        if 'bert' in model and 'chinese' not in model:
            tokenizer, _ = load_transformer_model_tokenizer(ppb.BertModel, ppb.BertTokenizer, '/home/public/gl/MultiDetection/PromptADDetection/plm/bert-base-uncased/')
        elif 't5' in model:
