...
def train_process(args):
    ...
    model_dict = {
        ...
        'roberta_mlm_pure_normal_100': os.path.join(args.off_line_model_dir, 'roberta_mlm_pure_normal_100'),
        ...
        'roberta_mlm_pure_normal_description': os.path.join(args.off_line_model_dir,'roberta_mlm_pure_normal_description'),
...
class TrainingArguments:
    def __init__(self):
        ...
        self.model_name = 'roberta_mlm_pure_normal_100'
    ...
    for d in dataset_type:
        ...
        for j in template_id:
            ...
            for training_arguments in training_arguments_list:
                ...
                training_arguments.model_name = 'roberta_mlm_pure_normal_description'   # 修改对应的模型类型
            ...
            # 将结果写入文件
            with open('/home/public/gl/MultiDetection/PromptADDetection/model_log/template_results_change_template.txt', 'a+') as f:
                # 遍历每个模板的结果
                f.write(f'datasetType_{d}_Template {j}:\n')
                for metric in average_results:
                    # 写入每个指标的平均结果
                    f.write(str(metric))
                    f.write(' ')
                f.write('\n')  # 写入空行分隔不同的模板结果
            # Append average results to the end of the results list
