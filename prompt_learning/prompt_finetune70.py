...
def train_process(args):
    ...
    ckpt_dir = f"{logs_dir}/{args.train_mode}/checkpoints/"
    ...
    if args.crossvalidation:
        ...
        # 加载手录的数据
        dataset['train'], train_classes_count = loading_data_asexample(data_dir, SAMPLE_SIZE, class_labels,
                                                                       args.model_name, mode=args.train_mode,args = args)
    ...
    model_dict = {
        ...
        'roberta-origin': r'/home/public/gl/MultiDetection/PromptADDetection/plm/roberta',
        ...
        'roberta_mlm_pure_normal_description_diff_order': os.path.join(args.off_line_model_dir, 'roberta_mlm_pure_normal_description_diff_order'),
        'roberta_mlm_pure_normal_description_5': os.path.join(args.off_line_model_dir, 'roberta_mlm_pure_normal_description_5'),
    ...
    def test_evaluation(prompt_model, ckpt_dir, dataloader, epoch_num=None):
        ...
        return  test_acc,  test_prec_macro,  test_recall_macro,  test_f1_macro
    ...
    results = None
    ckpt_dir = r'/home/public/gl/MultiDetection/PromptADDetection/model_log/parttuning/roberta-origin_tempmanual6_verbmanual_full_lr1e-05_layernum0/version_1/train100/checkpoints'
    if args.run_evaluation:
        logger.warning("Running evaluation on test set using best checkpoint!")
        print('seed', args.seed)
        for epoch_num in [ 1]:
            results = test_evaluation(prompt_model, ckpt_dir, test_data_loader, epoch_num)
...
class TrainingArguments:
    def __init__(self):
        self.seed = 2
        self.template_id = 1
        self.train_mode = 'train10'
        self.model = 'roberta'
        # self.model_name = 'roberta'
        self.model_name = 'roberta_mlm_pure_normal_description'

        self.plm_eval_mode = False
        self.tune_plm = True
        self.part_tuning = True
        self.freeze_verbalizer_plm = False
        self.project_root = '/home/public/gl/MultiDetection/PromptADDetection/'
        self.logs_root = '/home/public/gl/MultiDetection/PromptADDetection/model_log/'
        self.off_line_model_dir = '/home/public/gl/MultiDetection/PromptADDetection/plm'
        self.scripts_path = "template"
        self.template_type = "manual"
        self.use_pic_type = False
        self.verbalizer_type = "manual"
        self.manual_type = "A"
        self.data_dir = '/home/public/gl/MultiDetection/PromptADDetection/data/adress'
        self.data_not_saved = True
        self.batch_size = 1
        self.num_epochs = 10
        self.max_steps = 5000
        self.plm_lr = 1e-5
        self.plm_warmup_steps = 5
        self.prompt_lr = 0.3
        self.warmup_step_prompt = 5
        self.init_from_vocab = False
        self.eval_every_steps = 100
        self.soft_token_num = 5
        self.optimizer = "adamw"
        self.gradient_accum_steps = 1
        self.dev_run = False
        self.gpu_num = 0
        self.balance_data = False
        self.ce_class_weights = False
        self.sampler_weights = False
        self.training_size = "full"
        self.no_ckpt = False
        self.last_ckpt = True
        self.crossvalidation = False
        self.val_file_dir = r'/home/public/gl/MultiDetection/PromptADDetection/latest_tmp_dir/ten_fold_1.json'
        self.val_fold_idx = 999
        self.tensorboard = False
        self.tunable_layernum = 0
        self.pause = False
        self.pause_threshold = None
        self.transcription = 'chas'
        self.asr_format = 3
        self.dataset_name = "ADReSS"
        self.model_parallelize = True
    ...
    def __repr__(self):
        return f"TrainingArguments({vars(self)})"

def modify_seed(training_args_instance, new_seed):
    training_args_instance.seed = new_seed
...
if __name__ == '__main__':
    ...
    # template_id = [7,8,9]
    # template_id = [ 3, 4, 5, 6]
    # dataset_type = ['train10','train20','train30','train40','train50','train60','train70','train80','train90','train100']
    dataset_type = ['train100']
    dataset_res = []
    for d in dataset_type:
        template_res = []
        for j in template_id:
            seeds = [1,14,34,45,767,123,345,85,58,653]
            training_arguments_list = [modify_seed(TrainingArguments(), seed) for seed in seeds]
            indicator_name = ["test_acc",  "test_prec_macro",  "test_recall_macro",  "test_f1_macro"]
            results = []

            # 修改并尝试不同的样本进行训练
            for training_arguments in training_arguments_list:
                print(training_arguments)
                training_arguments.template_id = j   # 修改对应的模板id
                training_arguments.train_mode = d
                # training_arguments.model_name = 'roberta'   # 修改对应的模型类型
                training_arguments.model_name = 'roberta-origin'   # 修改对应的模型类型
                training_arguments.num_epochs = 1
                results.append(train_process(training_arguments))

            # 求出最终的平均结果，在保存一下
            average_results = [round(sum(metric) / len(results), 4) for metric in zip(*results)]
            template_res.append(average_results)
            with open('/home/public/gl/MultiDetection/PromptADDetection/model_log/roberta_mlm_pure_normal_diff_8_change_template.txt', 'a+') as f:
                # 遍历每个模板的结果
                f.write(f'Template {j}:\n')
                for metric in average_results:
                    # 写入每个指标的平均结果
                    f.write(str(metric))
                    f.write(' ')
                f.write('\n')
            # Print out the results for each seed and the average results at the end
            # for i, result in enumerate(results[:-1]):
            #     print(
            #         f"{seeds[i]:.2f} {result[0]:.2f} {result[1]:.2f} {result[2]:.2f} {result[3]:.2f}")

        dataset_res.append(template_res)
    print("template_id test_acc test_prec_macro test_recall_macro test_f1_macro")
    for i in range(len(dataset_res)):
        print(f"dataset_id: {i}")
        for j in range(len(dataset_res[i])):
            print(f"template_id: {j}")
            print(
                f"{dataset_res[i][j][0]:.2f} {dataset_res[i][j][1]:.2f} {dataset_res[i][j][2]:.2f} {dataset_res[i][j][3]:.2f}"
            )
