...
parser.add_argument("--val_file_dir", type=str)
...
def loading_data_asexample(data_save_dir, sample_size, classes, model, mode='train', validation_dict=None):
    """
    PS: Load data from the saved csv files.
...
# 这个是openprompt中专有的模型，加载专门针对语言训练的模型
...
if DATASET == "ADReSS":
    ...
    # 根据训练集中各类别的数量计算类别权重，以用于训练过程中的损失函数，缓解类别不平衡的问题
...
# Note that soft template can be combined with hard template,
# by loading the hard template from file.
...
def train(prompt_model, train_data_loader, num_epochs, mode = "train", ckpt_dir = ckpt_dir):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        val_loss, val_acc, val_prec_weighted, val_prec_macro, val_recall_weighted,val_recall_macro, val_f1_weighted,val_f1_macro, val_auc_weighted,val_auc_macro = evaluate(prompt_model, validation_data_loader, epoch=epoch)
        if not args.no_tensorboard:
